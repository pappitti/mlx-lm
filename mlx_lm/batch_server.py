...
from .generate import batch_generate, BatchStats, BatchResponse
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        endpoints = {
            ...
            "/v1/chat/completions": self.handle_batch_chat_completions,
            "/chat/completions": self.handle_batch_chat_completions,
        }
        ...
        prompts = endpoints[self.path]()
        self.handle_completion(prompts, stop_id_sequences)
    ...
    def handle_completion(
        ...
        if self.stream:
            raise ValueError("Streaming not supported for batch completions")
        else:
        ...
        batch_generation = batch_generate(
            model=self.model,
        ...
        stats = {
            "prompt_tokens": int(batch_generation.stats.prompt_tokens),
            "prompt_tps": float(batch_generation.stats.prompt_tps),
            "prompt_time": float(batch_generation.stats.prompt_time),
            "generation_tokens": int(batch_generation.stats.generation_tokens),
            "generation_tps": float(batch_generation.stats.generation_tps),
            "generation_time": float(batch_generation.stats.generation_time),
            "peak_memory": float(batch_generation.stats.peak_memory),
        }

        response = { "texts": batch_generation.texts, "stats": stats }
    ...
    def handle_batch_chat_completions(self) -> List[List[int]]:
        """
        Handle a batch chat completion request.

        Returns:
            List[mx.array]: A list of mx.array of the tokenized prompts from the request body
        """
        body = self.body
        assert "messages" in body, "Request did not contain messages"
        messages = body["messages"]
        if not isinstance(messages, list) or not all(
            isinstance(m, list) for m in messages
        ):
            raise ValueError("messages must be a list of list of message dicts")
        ...
        for msg in messages:
            ...
            if self.tokenizer.chat_template:
                ...
                raise ValueError("chat_template not found in tokenizer")
